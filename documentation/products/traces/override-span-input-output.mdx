...
- Pass these via the `attributes` argument of `update_current_span` (not `respan_params`).
- Values must be JSON‑serializable strings, so use `json.dumps(...)`.
- Keep any custom metadata in `respan_params["metadata"]` as needed.
...
<Note>
Use `update_current_span(attributes=...)` to override the span’s displayed input/output. `respan_params` do not map `input` or `output` to span attributes; they’re for metadata and Respan-specific options. After overriding, the trace view will display your custom values for the selected span.
</Note>