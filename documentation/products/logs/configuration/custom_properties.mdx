...
<Steps>
  <Step title="Get unique_id of the log">
    You need first to get the `unique_id` of the log you want to update. You can get the `unique_id` from the [Logs list endpoint](/api-endpoints/observe/logs/list). OR you can get the `unique_id` from [Logs page](https://platform.respan.ai/platform/requests)'s `Log ID` column.
