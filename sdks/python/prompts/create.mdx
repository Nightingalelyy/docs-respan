...
The `create()` and `acreate()` methods allow you to create a new prompt in Respan. A prompt serves as a container for multiple versions with different configurations. Use `create()` for synchronous operations and `acreate()` for asynchronous operations.
