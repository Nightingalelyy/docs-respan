...
To use [Respan parameters](/api-endpoints/develop/gateway/chat-completions#respan-parameters), you can pass them in the `extra_body` parameter if you're using the OpenAI SDK.
...
## Respan parameters
See how to make a standard Respan API call in the [Gateway Quickstart](/get-started/quickstart/gateway) guide.
...
<ParamField body="customer_params" type="object">
  Pass the customer's parameters in the API call to monitor the user's data in the Respan platform. See how to get insights into your users' data [here](/api-endpoints/observe/users/user-creation-endpoint)
  <Accordion title="Properties">
