...
api: PATCH https://api.respan.ai/api/models/{model_name}/
---
...
model_name = "my-custom-gpt-4"
url = f"https://api.respan.ai/api/models/{model_name}/"
headers = {
...
const url = `https://api.respan.ai/api/models/${modelName}/`;
const headers = {
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json'
};
...
```bash cURL
curl -X PATCH "https://api.respan.ai/api/models/my-custom-gpt-4/" \
