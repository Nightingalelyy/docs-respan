...
You can pass in a dictionary of your customer's credentials and deployment variables for [supported providers](/integration/overview) and use their credits when the router is calling models from those providers.
```json
{
  "openai": {
...
<ParamField path="exclude_models" type="array" default={[]}>
  The list of models to exclude from the router's selection. See the list of models [here](/integration/overview)
...
<ParamField path="exclude_providers" type="array" default={[]}>
  The list of providers to exclude from the router's selection. All models under the provider will be excluded. See the list of providers [here](/integration/overview)
