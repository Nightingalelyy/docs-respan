...
### Cons
- **No Direct Prompt Access**: You can't fetch prompts stored in Respan directly. You'll need to use the [Prompts API](/api-endpoints/develop/prompts/create-prompts) instead.
...
To use [Respan parameters](/api-endpoints/develop/gateway/chat-completions#respan-parameters), you can pass them in the `metadata` parameter. In the above example, `customer_identifier` is a Respan parameter. These parameters will take precedence over the Anthropic parameters if they are conflicting.
